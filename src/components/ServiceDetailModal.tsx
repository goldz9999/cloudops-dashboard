  ...
  return (
    ...
    >
      <div
        ...
        className="bg-white rounded-xl border border-[#E2E8F0] shadow-xl w-full max-w-lg max-h-[85vh] flex flex-col"
      ...
      >
        <div className="flex items-start justify-between gap-3 px-5 py-4 border-b border-[#E2E8F0]">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 id="service-detail-title" className="text-base font-semibold text-[#1E293B]">
                {svc.name}
              </h2>
          ...
          <button
            onClick={onClose}
            aria-label="Cerrar"
            className="p-1.5 rounded-lg hover:bg-slate-100 text-[#64748B] shrink-0"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="overflow-y-auto p-5 space-y-4">
          <div className="flex items-center justify-between">
            <span
              className={`inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-full ${
                inUse ? 'bg-green-50 text-[#16A34A]' : 'bg-slate-100 text-[#64748B]'
              }`}
            >
          ...
          <p className="text-sm text-[#1E293B] leading-relaxed">{svc.description}</p>
          ...
          <div className="grid grid-cols-2 gap-3 pt-1">
            <div className="bg-slate-50 rounded-lg p-3">
              ...
              <p className="text-lg font-semibold text-[#1E293B]">{metric?.resources ?? 0}</p>
            </div>
            <div className="bg-slate-50 rounded-lg p-3">
              <div className="flex items-center gap-1.5 text-[#64748B] text-xs mb-1">
                <Activity className="w-3.5 h-3.5" /> Uso
              </div>
              <p className="text-lg font-semibold text-[#1E293B]">{inUse ? `${metric.usage}%` : '—'}</p>
            </div>
            <div className="bg-slate-50 rounded-lg p-3 col-span-2">
              ...
              <p className="text-lg font-semibold text-[#1E293B]">
                ...
                <span className="text-xs font-normal text-[#64748B]">({pctOfTotal}% del total de la región)</span>
              </p>
            </div>
          </div>
        </div>
